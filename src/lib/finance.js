...
import { emriIPlote, eshteMujore, familjaSet, kategoriteMujore, rrenjaE } from "./kategorite";
 ...
 *    allowance over money that was never part of it. The same goes for spending marked **monthly**
 *    rather than daily, on its category or on the transaction itself: a tank of fuel is three weeks
 *    of driving, and charging all of it to the day it was bought reports a blown day that never
 *    happened. Monthly spending still leaves the balance, so it makes every remaining day of the
 *    month a little tighter - which is exactly what it does in real life.
 */
export function dailyLimit({
  ...
  const shpenzuarSot = sotShpenzimet
    .filter((tx) => !eshteMujore(tx, mujoret))
    .reduce((sum, tx) => sum + toNumber(tx.vlera), 0);
